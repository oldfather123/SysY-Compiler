...
class IRBuilder {
  ...
  public:
    ...
    IBinaryInst *create_srem(Value *lhs, Value *rhs) {
    ...
    FBinaryInst *create_frem(Value *lhs, Value *rhs) {
        return FBinaryInst::create_frem(lhs, rhs, this->BB_);
    }
    BitCastInst *create_bitcast(Value *val, Type *dest_ty) {
        return BitCastInst::create_bitcast(val, dest_ty, this->BB_);
    }
};
