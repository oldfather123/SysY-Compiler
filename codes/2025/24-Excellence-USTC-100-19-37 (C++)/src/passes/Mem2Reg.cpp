...
void Mem2Reg::rename(BasicBlock *bb) {
    ...
    for(auto &inst:bb->get_instructions())
    {
        ...
        if(inst.is_phi())
        {
            auto lval=phi_lval[static_cast<PhiInst *>(&inst)];
            
            var_val_stack[lval].push_back(&inst);
        }
        // 步骤二：用 lval 最新的定值替代对应的load指令
        if(inst.is_load())
        {
            auto lval=static_cast<LoadInst*>(&inst)->get_lval();
            if(var_val_stack.find(lval)!=var_val_stack.end()&&is_valid_ptr(lval))
            {
        ...
        if(inst.is_store())
        {
            auto rval=static_cast<StoreInst*>(&inst)->get_rval();
    ...
    {
        for(auto &inst:nextbb->get_instructions())
        {
            if(inst.is_phi())
            {
                auto lval=phi_lval[static_cast<PhiInst *>(&inst)];
                if(var_val_stack.find(lval)!=var_val_stack.end())
                    static_cast<PhiInst *>(&inst)->add_phi_pair_operand(var_val_stack[lval].back(), bb);
            }
        }
    }

    // 步骤五：对 bb 在支配树上的所有后继节点，递归执行 re_name 操作
    for(auto nextbb:dominators_->get_dom_tree_succ_blocks(bb))
        rename(nextbb);
    // 步骤六：pop出 lval 的最新定值
    for(auto &inst:bb->get_instructions())
    {
