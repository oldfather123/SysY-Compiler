    ...
    while (!work_list.empty()) { // 当工作表非空时继续处理
        ...
        // TODO-2: 处理已属于其他循环的节点
            ...
            if(p==loop)
                continue;
            //4
            subloop->set_parent(loop);//设置父循环
            loop->add_sub_loop(subloop);//添加子循环
            //5
            for(auto &bb:subloop->get_blocks())
                loop->add_block(bb);
            //6
            for(auto &prebb:subloop->get_header()->get_pre_basic_blocks())
                work_list.push_back(prebb);
