...
FBinaryInst *FBinaryInst::create_frem(Value *v1, Value *v2, BasicBlock *bb) {
    return create(frem, v1, v2, bb);
}
...
BitCastInst::BitCastInst(Value *val, Type *ty, BasicBlock *bb)
    ...
    assert(val->get_type() != ty && "BitCastInst source and destination types are the same");
    add_operand(val);
}
