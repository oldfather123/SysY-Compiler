...
bool LoopUnroll::run()
{
  dom = AM.get<DominantTree>(m_func);
...
CallInst *LoopUnroll::ExtractLoopBody(LoopInfo *loop)
{
  ...
  loop->trait.res = res;
  IR_DataType ty = IR_DataType::IR_Value_VOID;
  if (res)
    ty = res->GetTypeEnum();

  auto UnrollFunc = std::make_unique<Function>(ty, Singleton<Module>().GetFuncNameEnum("Loop_Body"));
  Singleton<Module>().push_func(std::move(UnrollFunc)); // xxxxxx
  ...
  auto judge = [&](Value *target)
  {
    ...
    if (target->isConst() || target->isGlobal())
      return false;
    auto user = dynamic_cast<Instruction *>(target); // xxxxxx
    assert(user);
  ...
  for (const auto &[val, arg] : Val2Arg)
    args.push_back(val);
  auto callinst = new CallInst(UnrollFunc.get(), args); // xxxxxx
  substitute->push_back(callinst);
  auto loopchange = dynamic_cast<Instruction *>(loop->trait.change);
  assert(loopchange);
  OriginChange = loopchange;
  auto change = loopchange->CloneInst();
  substitute->push_back(change);
  loop->trait.change = change;
  auto loopcmp = dynamic_cast<Instruction *>(latch->GetBack()->GetOperand(0));
  assert(loopcmp);
  auto cmp = loopcmp->CloneInst();
  substitute->push_back(cmp);
  BasicBlock *_exit = nullptr;
  if (auto cond = dynamic_cast<CondInst *>(latch->GetBack()))
    for (int i = 1; i < 3; i++)
    {
  ...
  for (auto &use : cmp->GetUserUseList())
  {
    if (use->GetValue() == loopchange)
      cmp->ReplaceSomeUseWith(use.get(), change);
  }
  // 修改header和exit
  for (auto predNode : dom->getNode(loop->GetHeader())->predNodes)
  {
    if (predNode->curBlock != latch)
    {
      auto prehead = predNode->curBlock;
      if (auto cond = dynamic_cast<CondInst *>(prehead->GetBack()))
      {
        for (int i = 1; i < 3; i++)
        {
          if (cond->GetOperand(i) == header)
            cond->ReplaceSomeUseWith(i, substitute);
        }
      }
      else if (auto uncond = dynamic_cast<UnCondInst *>(prehead->GetBack()))
      {
        uncond->ReplaceSomeUseWith(0, substitute);
      }
    }
  }

  bool NoUse = false;
  for (auto succNode : dom->getNode(latch)->succNodes)
  {
    if (succNode->curBlock != header)
    {
      auto succBB = succNode->curBlock;
      for (auto it = succBB->begin();it != succBB->end() && dynamic_cast<PhiInst *>(*it);++it)
      {
        ...
        if (it1 != phi->PhiRecord.end())
        {
          ...
          {
            phi->ReplaceSomeUseWith(it1->first, change);
            it1->second.first = change;
          }
          else if (res && it1->second.first == res->ReturnValIn(latch))
          {
            phi->ReplaceSomeUseWith(it1->first, callinst);
          ...
          else if (!res && dynamic_cast<PhiInst *>(it1->second.first))
          {
            ...
            if (phi_res->GetParent() == latch)
            {
              ...
              phi_res->ReplaceSomeUseWith(it1->first, callinst);
              it1->second.first = callinst;
              NoUse = true;
            }
          }
          ...
          for (auto &use : inst->GetUserUseList())
          {
            if (Val2Arg.find(use->GetValue()) != Val2Arg.end())
            {
              if (auto phi = dynamic_cast<PhiInst *>(inst))
                ...
                inst->ReplaceSomeUseWith(use.get(), Val2Arg[use->GetValue()]);
            }
          }
        }
      if (res && !NoUse)
        res->addIncoming(callinst, substitute);
      loop->trait.indvar->addIncoming(loop->trait.change, substitute);
      // 转移
      substitute->num = header->num;
      m_func->GetBBs()[header->num] = std::make_unique<BasicBlock>(*substitute);
      dom->getNode(header)->curBlock = substitute;
...
BasicBlock *LoopUnroll::Unroll(LoopInfo *loop, CallInst *UnrollBody)
{
  ...
  BasicBlock::List<BasicBlock, Instruction>::iterator call_pos(UnrollBody); // xxxxxx
  std::vector<Instruction *> Erase;                                         // xxxxxx
  Instruction *cloned = UnrollBody;
  ...
  if (ResOrigin)
    UnrollBody->ReplaceAllUseWith(ResOrigin);
  ...
  for (auto use : UnrollBody->GetValUseList())
  {
    ...
    if (userInst->GetParent() == exit)
      userInst->ReplaceSomeUseWith(use, ResOrigin);
  }
...
bool LoopUnroll::CanBeHalfUnroll(LoopInfo *loop)
{
  ...
  if (dynamic_cast<PhiInst *>(loop->trait.boundary))
    return false;
  if (auto user = dynamic_cast<Instruction *>(loop->trait.boundary)) // xxxxxx
    if (loop->Contain(user->GetParent()))
      return false;
...
BasicBlock *LoopUnroll::Half_Unroll(LoopInfo *loop, CallInst *UnrollBody)
{
  ...
  m_func->PushBothBB(Unroll_Entry);
  m_func->PushBothBB(MutiUnrollBlock);
  m_func->PushBothBB(SingleHeader);
  ...
  BasicBlock::List<BasicBlock, Instruction>::iterator call_pos(new_call);
  std::vector<User *> Erase;
  Instruction *cloned = new_call;
  Erase.push_back(cloned);
  Value *CurChange = nullptr;
