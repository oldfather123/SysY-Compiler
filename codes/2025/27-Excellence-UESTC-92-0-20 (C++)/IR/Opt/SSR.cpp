#include "../../include/IR/Opt/SSR.hpp"
...
bool ScalarStrengthReduce::RunOnLoop(LoopInfo *loop)
{
    ...
    if (Loop->GetExit(loop).size() != 1)
        return false;
    if (loop->GetHeader()->GetValUseList().GetSize() != 2)
        return false;
    ...
    for (Instruction *inst : *header)
    {
        if (auto phi_ = dynamic_cast<PhiInst *>(inst))
        {
            ...
            {
                auto User_Add = dynamic_cast<Instruction *>(user->GetUser());
                if (!User_Add)
                    continue;

                if (User_Add->GetInstId() == Instruction::Op::Add)
                {
                    Value *LHS = User_Add->GetOperand(0);
                    Value *RHS = User_Add->GetOperand(1);
                    Add_RHS = (LHS == inst) ? RHS : LHS;
                    Add = User_Add;

                    for (auto user_user : User_Add->GetValUseList())
                    {
                        auto User_Rem = dynamic_cast<Instruction *>(user_user->GetUser());
                        if (!User_Rem)
                            continue;

                        if (User_Rem->GetInstId() == Instruction::Op::Mod)
                        {
                            Value *LHS_Rem_ = User_Rem->GetOperand(0);
                            Value *RHS_Rem_ = User_Rem->GetOperand(1);
                            Rem_RHS = (LHS_Rem_ == User_Add) ? RHS_Rem_ : LHS_Rem_;
                            Rem = User_Rem;
                        }

                        for (auto user_user_user : User_Rem->GetValUseList())
                        {
                            auto userInst = dynamic_cast<Instruction *>(user_user_user->GetUser());
                            if (!userInst)
                                continue;
...
Value *ScalarStrengthReduce::CaculateTimes(LoopInfo *loop)
{
        ...
        dynamic_cast<BinaryInst *>(loop->trait.change))
    {
        ...
        BinaryInst::Operation Op = binary->GetOp();
    ...
    else if (dynamic_cast<BinaryInst *>(loop->trait.change))
    {
        ...
        BinaryInst::Operation Op = change->GetOp();
        if (dynamic_cast<BinaryInst *>(bound) && dynamic_cast<BinaryInst *>(initial))
        {
            ...
            if (bound_binary->GetOp() == BinaryInst::Op_Add &&
                initial_binary->GetOp() == BinaryInst::Op_Add)
            {
            ...
            if (auto INT = dynamic_cast<ConstIRInt *>(initial))
            {
                if (INT->GetVal() == 0)
                {
                    ...
                    case BinaryInst::Op_Sub:
                        ...
                        {
                            if (bound->GetType()->GetTypeEnum() == IR_DataType::IR_Value_INT)
