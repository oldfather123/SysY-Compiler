...
public class ARMVecMoveToSca extends ARMIns {
    private VirFReg src0;
    private final int index;

    public ARMVecMoveToSca(VirFReg src0, int index, ASMBasicBlock parentBlock, Reg register) {
    ...
    public void resetOperands(List<ASMValue> values) {
        if (values == null || values.size() != 1) {
            throw new RuntimeException("加载向量到标量指令接受且只接受一个操作数");
        }

        this.modifyUse(src0, values.getFirst());
        this.src0 = (VirFReg) values.getFirst();
    }

    @Override
    protected String printIns() {
        if (!(this.src0.getPhyReg() instanceof PhyFReg fReg)) {
            throw new IllegalArgumentException("The PhyReg of operands must be of type PhyFReg and PhyFReg");
        }

        return "MOV " + this.printAsOperand() + ", " + fReg.printAsPartNOfVec(this.index);
    }
}
