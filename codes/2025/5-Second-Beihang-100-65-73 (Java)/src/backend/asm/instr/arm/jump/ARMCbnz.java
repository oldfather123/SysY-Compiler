 ...
 */
public class ARMCbnz extends ARMIns implements BranchIns {
    private Reg operand;
    private final ASMBasicBlock target;

    public ARMCbnz(ASMBasicBlock target, Reg operand, ASMBasicBlock parentBlock) {
    ...
    public void resetOperands(List<ASMValue> values) {
        if (values == null || values.size() != 1) {
            throw new RuntimeException("CBNZ 指令接受且只接受一个操作数");
        }
    ...
    protected String printIns() {
        return "CBNZ " + operand.printAsOperand() + ", " + target.printAsOperand();
    }
}
