...
//                Value hashVal2 = hashCal(hashBlk, function, castArgs, modVal2, baseVal2);
                    ...
                    CmpInstr usedCond = new CmpInstr(function.getAndAddRegIdx(), CmpInstr.CmpCond.INE,
                            usedVal, castArgs.getFirst(), hashBlk);
                    new BranchInstr(usedCond, blks.get(1), loadBlk, hashBlk); // 分别对应要递归计算和不要递归计算
                    ...
                    StoreInstr storeUsed = new StoreInstr(castArgs.getFirst(), usedPtr, blks.get(1));
                    storeUsed.setUse(castArgs.getFirst());
