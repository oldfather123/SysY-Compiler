...
bool ir::Executor::exec_ir(size_t n) {
    while (n--) {
        ...
        if (cur_ctx->pc < cur_ctx->pfunc->InstVec.size()) {
            inst = cur_ctx->pfunc->InstVec[cur_ctx->pc];
        } else if (cur_ctx->pc == cur_ctx->pfunc->InstVec.size()) {
            // if pc is at the end of the function, then return
            if (cur_ctx->pfunc->returnType == ir::Type::null) {  // void function
                inst = new ir::Instruction(ir::Operand(), ir::Operand(), ir::Operand(), ir::Operator::_return);
            } else if (cur_ctx->pfunc->returnType == ir::Type::Int) {  // int function
                inst = new ir::Instruction(ir::Operand("0", ir::Type::IntLiteral), ir::Operand(), ir::Operand(), ir::Operator::_return);
            } else if (cur_ctx->pfunc->returnType == ir::Type::Float) {  // float function
                inst = new ir::Instruction(ir::Operand("0.0", ir::Type::FloatLiteral), ir::Operand(), ir::Operand(), ir::Operator::_return);
            }
        } else {
            std::cout << "pc out of range: " << cur_ctx->pc << " >= " << cur_ctx->pfunc->InstVec.size() << std::endl;
            assert(0 && "pc out of range");
        }
        ...
        case Operator::alloc: {
            ...
            if (inst->des.type == Type::IntPtr) {
                get_des_operand(inst->des)->_val.iptr = new int[size]();  // init to 0
        ...
        case Operator::_or: {
            ...
            if (IS_INT_OPERAND(inst->op1)) {
                ...
                std::cout << "type of " << inst->op1.name << " is " << toString(inst->op1.type) << std::endl;
        ...
        case Operator::fneq: {
            ...
            //     std::cout << "inst: " << inst->draw() << std::endl;
            // }
            // assert(inst->des.type == Type::Float);
            // op1
            float v1;
...
void putint(int a) { fprintf(ir::reopen_output_file, "%d", a); }

void putch(int a) { fprintf(ir::reopen_output_file, "%c", a); }
...
void putfloat(float a) { fprintf(ir::reopen_output_file, "%a", a); }
