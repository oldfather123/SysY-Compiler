...
for sy in "$dir"/*.sy; do
  ...
  if timeout 30s "$compiler" "$sy" >/dev/null 2>&1 && \
    ...
    dos2unix "$base.tmp" >/dev/null 2>&1 #brainfk这个测试中会打印/r/n，但是我们是linux
    if diff -q "$base.tmp" "$exp" >/dev/null; then
      elapse=$(awk "BEGIN {print $end - $start}")
      echo "$(basename "$base"): SUCC" $elapse
      pass=$((pass+1))
    else
