...
unsigned Type::get_bit_width() const {
    switch (get_type_id()) {
    case IntegerTyID:
        if (is_int1_type())
            return 1;
            ...
            assert(false && "unsupported int bit width");
    case FloatTyID:
        return 32;
    // 如果支持 double 类型：
    // case DoubleTyID:
    //     return 64;
    default:
        assert(false && "get_bit_width: unsupported type");
    }
}
...
bool FunctionType::is_valid_argument_type(Type *ty) {
    return ty->is_integer_type() || ty->is_pointer_type() ||
           ty->is_float_type()||ty->is_void_type();
}//main函数怎么接受参数，考虑专门写一个
