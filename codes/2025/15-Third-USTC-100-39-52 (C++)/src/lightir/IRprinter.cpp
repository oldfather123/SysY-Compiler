...
std::string print_binary_inst(const BinInst &inst)
{
    ...
    instr_ir += " ";
    // if(inst.get_operand(0)->get_type()==""){}
...
std::string MqtInst::print() {
    std::string instr_ir;

    // 指令左值（返回值）
    instr_ir += "%";
    instr_ir += get_name();   // 假设 get_name() 返回 %op3
    instr_ir += " = ";
    ...
    // 基地址，带 %
    instr_ir += print_as_op(get_ptr(), true);
    instr_ir += " ";

    // 偏移量，不带 %
    instr_ir += print_as_op(get_offset(), false);
