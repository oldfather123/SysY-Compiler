...
IBinaryInst *IBinaryInst::create_sand(Value *v1, Value *v2, BasicBlock *bb)
{
    return create(sand, v1, v2, bb);
}
...
// ICmpInst *ICmpInst::create_iiadd(Value *v1, Value *v2, BasicBlock *bb)
// {
//     return create(iiand, v1, v2, bb);
// }
...
{
    static const std::array<Type::TypeID, 4> allowed_alloc_type = {
...
void PhiInst::add_phi_pair_operand(Value *val, Value *pre_bb)
{
    ...
    this->add_incoming(val, dynamic_cast<BasicBlock *>(pre_bb)); // ✅ 现在不会报错了
}

void PhiInst::replace_phi_operand(Value *from, Value *to, BasicBlock *to_bb)
{
    for (auto &p : incoming)
    {
        if (p.first == from)
...
AndInst::AndInst(OpID id, Value *lhs, Value *rhs, BasicBlock *bb)
    : BaseInst<AndInst>(bb->get_module()->get_int1_type(), id, bb)
{
    // assert(lhs->get_type()->is_int32_type() &&
    //        rhs->get_type()->is_int32_type() &&
    //        "CmpInst operands are not both i32");
...
AndInst *AndInst::create_iiand(Value *v1, Value *v2, BasicBlock *bb){
    return create(iiand, v1, v2, bb);
}

MqtInst::MqtInst(Value *base_ptr, Value *offset, BasicBlock *bb)
    : BaseInst<MqtInst>(base_ptr->get_type(), Instruction::mqt, bb) 
{
    // --- 合法性检查 ---
    assert(base_ptr->get_type()->is_pointer_type() &&
            "mqt base operand must be a pointer (array address)");

    assert(offset->get_type()->is_integer_type() &&
            "mqt offset must be an integer");
    assert(dynamic_cast<ConstantInt *>(offset) != nullptr &&
            "mqt offset must be a ConstantInt");
...
MqtInst *MqtInst:: create_mqt(Value *base_ptr, Value *offset, BasicBlock *bb) {
        return create(base_ptr, offset, bb);
}