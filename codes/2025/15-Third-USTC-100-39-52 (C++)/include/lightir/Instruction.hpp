...
class Instruction : public User,  public ilist<Instruction>::node {
  public:
    ...
    bool is_sand() const { return op_id_ == sand; }
    ...
    bool is_iiand() const { return op_id_ == iiand; }
    ...
    bool isBinary() const {
        return (is_add() || is_sub() || is_mul() || is_div() || is_srem() || is_fadd() ||
                is_fsub() || is_fmul() || is_fdiv()||is_shl()||is_ashr()||is_sand()) &&
               (get_num_operand() == 2);
    }
...
class IBinaryInst : public BaseInst<IBinaryInst> {
  ...
  public:
    ...
    static IBinaryInst *create_sand(Value *v1, Value *v2, BasicBlock *bb);
...
class ICmpInst : public BaseInst<ICmpInst> {
  ...
  public:
    ...
    // static ICmpInst *create_iiadd(Value *v1, Value *v2, BasicBlock *bb);


    virtual std::string print() override;
};

class AndInst :  public BaseInst<AndInst> {
  friend BaseInst<AndInst>;

  private:
    AndInst(OpID id, Value *lhs, Value *rhs, BasicBlock *bb);
  public:
  static AndInst *create_iiand(Value *v1, Value *v2, BasicBlock *bb);
...
class BranchInst : public BaseInst<BranchInst> {
  ...
  public:
    BasicBlock* get_true_block() const {
        return reinterpret_cast<BasicBlock*>(get_operand(1));
    }
    BasicBlock* get_false_block() const {
        return reinterpret_cast<BasicBlock*>(get_operand(2));
    }
...
public:
  ...
  void add_phi_pair_operand(Value *val, Value *pre_bb);
  virtual std::string print() override;
  void replace_phi_operand(Value *from, Value *to, BasicBlock *to_bb);
...
public:
  ...
  Type *get_dest_type() const { return get_type(); };
  // static BitCastInst *clone_BitCastInst(BitCastInst *inst, BasicBlock *bb);
  virtual std::string print() override;
};

class MqtInst : public BaseInst<MqtInst> {
    friend BaseInst<MqtInst>;

  private:
    //mqt 接收 (ptr,offset)
    MqtInst(Value *ptr, Value *offset, BasicBlock *bb);

  public:
    static MqtInst *create_mqt(Value *val, Value *const_val, BasicBlock *bb);

    Value *get_ptr() const { return this->get_operand(0); }
    Value *get_offset() const { return this->get_operand(1); }
