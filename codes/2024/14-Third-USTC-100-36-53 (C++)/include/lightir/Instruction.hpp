...
class Instruction : public User, public ADT::ilist_node<Instruction> {
  public:
    enum OpID : uint32_t {
    ...
    void insert_before(Instruction *after);
    ...
    bool is_communicative() const {
        return (is_add() || is_mul() || is_fadd() || is_fmul() ||
                op_id_ == eq || op_id_ == ne || op_id_ == feq ||
                op_id_ == fne) &&
               get_num_operand() == 2;
    }

    bool is_terminator() const { return is_br() || is_ret(); }
...
class GetElementPtrInst : public BaseInst<GetElementPtrInst> {
  ...
  public:
    ...
    static GetElementPtrInst *clone_GetElementPtrInst(GetElementPtrInst *inst,
                                                      BasicBlock *bb);
    virtual std::string print() override;
};
...
class PhiInst : public BaseInst<PhiInst> {
  ...
  public:
    ...
    std::vector<std::pair<Value *, BasicBlock *>> get_pairs() const;
