...
void Instruction::insert_before(Instruction *after) {
    auto bb = after->get_parent();
    set_parent(bb);
    bb->get_instructions().insert_before(this, after);
}

Instruction *Instruction::clone_inst(Instruction *inst, BasicBlock *bb,
                                     bool before_terminator) {
    Instruction *new_inst;
    switch (inst->get_instr_type()) {
    // Terminator Instructions
    case ret:
        new_inst =
            ReturnInst::clone_ReturnInst(dynamic_cast<ReturnInst *>(inst), bb);
        break;
    case br:
        new_inst =
            BranchInst::clone_BranchInst(dynamic_cast<BranchInst *>(inst), bb);
        break;
    ...
    case sdiv:
    case srem:
        new_inst = IBinaryInst::clone_IBinaryInst(
            dynamic_cast<IBinaryInst *>(inst), bb);
        break;
    ...
    case fmul:
    case fdiv:
        new_inst = FBinaryInst::clone_FBinaryInst(
            dynamic_cast<FBinaryInst *>(inst), bb);
        break;
    // Memory operators
    case alloca:
        new_inst =
            AllocaInst::clone_AllocaInst(dynamic_cast<AllocaInst *>(inst), bb);
        break;
    case load:
        new_inst = LoadInst::clone_LoadInst(dynamic_cast<LoadInst *>(inst), bb);
        break;
    case store:
        new_inst =
            StoreInst::clone_StoreInst(dynamic_cast<StoreInst *>(inst), bb);
        break;
    ...
    case eq:
    case ne:
        new_inst = ICmpInst::clone_ICmpInst(dynamic_cast<ICmpInst *>(inst), bb);
        break;
    ...
    case feq:
    case fne:
        new_inst = FCmpInst::clone_FCmpInst(dynamic_cast<FCmpInst *>(inst), bb);
        break;
    // Other operators
    case phi:
        new_inst = PhiInst::clone_PhiInst(dynamic_cast<PhiInst *>(inst), bb);
        bb->add_instruction(new_inst);
        break;
    case call:
        new_inst = CallInst::clone_CallInst(dynamic_cast<CallInst *>(inst), bb);
        break;
    case getelementptr:
        new_inst = GetElementPtrInst::clone_GetElementPtrInst(
            dynamic_cast<GetElementPtrInst *>(inst), bb);
        break;
    case zext:
        new_inst = ZextInst::clone_ZextInst(dynamic_cast<ZextInst *>(inst), bb);
        break; // zero extend
    case fptosi:
        new_inst =
            FpToSiInst::clone_FpToSiInst(dynamic_cast<FpToSiInst *>(inst), bb);
        break;
    case sitofp:
        new_inst =
            SiToFpInst::clone_SiToFpInst(dynamic_cast<SiToFpInst *>(inst), bb);
        break;
    case bitcast:
        new_inst = BitCastInst::clone_BitCastInst(
            dynamic_cast<BitCastInst *>(inst), bb);
        break;
    default:
        ASSERT(false && "cloning unknow instructuions");
    }
    if (before_terminator) {
        for (auto it = bb->get_instructions().begin();
             it != bb->get_instructions().end(); ++it) {
            if (it->is_terminator()) {
                bb->get_instructions().insert(it, new_inst);
                break;
            }
        }
    }
...
IBinaryInst::IBinaryInst(OpID id, Value *v1, Value *v2, BasicBlock *bb)
    : BaseInst<IBinaryInst>(v1->get_type(), id, bb) {
...
IBinaryInst *IBinaryInst::clone_IBinaryInst(IBinaryInst *inst, BasicBlock *bb) {
    return create(inst->get_instr_type(), inst->get_operand(0),
                  inst->get_operand(1), bb);
}
...
BranchInst *BranchInst::clone_BranchInst(BranchInst *inst, BasicBlock *bb) {
    if (inst->is_cond_br())
        return create(inst->get_operand(0),
                      dynamic_cast<BasicBlock *>(inst->get_operand(1)),
                      dynamic_cast<BasicBlock *>(inst->get_operand(2)), bb);
    else
        return create(nullptr, dynamic_cast<BasicBlock *>(inst->get_operand(0)),
                      nullptr, bb);
}
...
GetElementPtrInst *
GetElementPtrInst::clone_GetElementPtrInst(GetElementPtrInst *inst,
                                           BasicBlock *bb) {
    std::vector<Value *> idxs;
    for (unsigned i = 1; i < inst->get_num_operand(); i++)
...
std::vector<std::pair<Value *, BasicBlock *>> PhiInst::get_pairs() const {
    std::vector<std::pair<Value *, BasicBlock *>> ret;
    for (auto it = get_operands().begin(); it != get_operands().end();
         it += 2) {
        auto op = *it;
        auto bb = *(it + 1);
...
BitCastInst *BitCastInst::clone_BitCastInst(BitCastInst *inst, BasicBlock *bb) {
    return create(inst->get_operand(0), inst->get_dest_type(), bb);
}