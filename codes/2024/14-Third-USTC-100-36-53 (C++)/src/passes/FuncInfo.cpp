...
void FuncInfo::run(Module *m, AnalysisPassManager &APM) {
    for (auto &f : m->get_functions()) {
        auto func = &f;
        trivial_mark(func);
        check_recursive(func);
        if (not is_pure[func])
...
void FuncInfo::check_recursive(Function *func) {
    if (func->is_declaration() or func->get_name() == "main") {

        is_recursive[func] = false;
        return;
    }
    for (auto &bb : func->get_basic_blocks())
        for (auto &inst : bb.get_instructions()) {
            if (inst.is_call() and inst.get_operand(0) == func) {
                is_recursive[func] = true;
                return;
            }
        }
    is_recursive[func] = false;
