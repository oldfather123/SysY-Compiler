#define NDEBUG
#include "../../../include/pass/optimize/reassociate.hpp"
...
void reassociatePass::run(ir::Function* func, topAnalysisInfoManager* tp){
    ...
    for(auto bb:func->blocks())vis[bb]=false;
...
void reassociatePass::DFSPostOrderBB(ir::BasicBlock*bb){
    vis[bb]=true;
    for(auto bbNext:bb->next_blocks()){
        if(not vis[bb])
            DFSPostOrderBB(bbNext);
    }
