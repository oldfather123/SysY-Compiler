...
// 这里将string_view作为返回值是很危险的，必须小心处理
std::string_view op_string(UnaryOp op) {
  unsigned i = static_cast<unsigned>(op);
  if(i >= 3) return "<unknown_unary_op>";
  static constexpr std::string_view op_strs_unary[] = { "+", "-", "!" };
  return op_strs_unary[i];
}
...
std::string_view op_string(BinaryOp op) {
  ...
  static constexpr std::string_view op_strs_binary[] = {
      "+", "-", "*", "/", "%", "==", "!=", "<", ">", "<=", ">=", "&&", "||"};
  return op_strs_binary[i];
}
...
ostream &operator<<(ostream &os, const ArrayType &type) {
  ...
  if (type.first_dimension_omitted()) n_dims++;
...
void UnaryExpr::print(std::ostream &out, unsigned int indent) const {
  ...
  assert(m_operand), "Error: No operand,this should not happen!";

  m_operand->print(out, indent + INDENT_LEN);//最后会带一个换行符
}

void BinaryExpr::print(std::ostream &out, unsigned int indent) const {
  print_indent(out, indent);
  out << "BinaryExpr " << op_string(m_op) << '\n';
  assert(m_lhs), "Error:no lhs,this should not happen!";
  assert(m_rhs), "Error:no rhs,this should not happen!";
  m_lhs->print(out, indent + INDENT_LEN);//最后会带一个换行符
  m_rhs->print(out, indent + INDENT_LEN);//最后会带一个换行符
}
...
void Assignment::print(std::ostream &out, unsigned int indent) const {
  print_indent(out, indent);
  out << "Assignment (\n";
  assert(m_lhs), "Error in assignment print lhs,this should not happen!";
  m_lhs->print(out, indent + INDENT_LEN);
  assert(m_rhs), "Error in assignment print rhs,this should not happen!";
...
void IfElse::print(std::ostream &out, unsigned int indent) const {
  ...
  out << "Cond\n";
  assert(m_cond), "Error in Cond,this should not happen!";
  m_cond->print(out, indent + INDENT_LEN * 2);

  print_indent(out, indent + INDENT_LEN);
  out << "Then\n";
  assert(m_then), "Error in Then,this should not happen!";
...
void While::print(std::ostream &out, unsigned int indent) const {
  ...
  print_indent(out, indent + INDENT_LEN);
  out << "Body\n";
  assert(m_body), "Error in body,this should not happen!";
...
void Function::print(std::ostream &out, unsigned int indent) const {
  ...
  for (int i = 0; i < m_params.size(); ++i) {
    if (i)
      out << ", ";
    assert(m_params[i]), "Error in params No." + std::to_string(i) + ",this should not happen!";
    out << *m_params[i];
  }
  out << ")\n";
  assert(m_body), "Error in body,this should not happen!";
