...
std::string ConstantArray::print()
{
    ...
    LOG(INFO) << this->get_size_of_array();
    bool is_zero = true;
    if(!dynamic_cast<ConstantArray *>(get_element_value(0)))
        for(unsigned i = 0; i < this->get_size_of_array(); i++)
        {
            if (dynamic_cast<ConstantInt *>(get_element_value(i)))
            {
                if (dynamic_cast<ConstantInt *>(get_element_value(i))->get_value() != 0)
                {
                    is_zero = false;
                    break;
                }
    ...
    if(!dynamic_cast<ConstantArray *>(get_element_value(0)) && is_zero)
    {
        // const_ir += get_element_value(0)->get_type()->print();
        const_ir += " zeroinitializer";
    }
    else{
        const_ir += "[";
        for (unsigned i = 0; i < this->get_size_of_array(); i++)
