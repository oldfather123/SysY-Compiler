...
public class LoadStoreDataflowAnalysis implements Pass.IRPass {

    LinkedHashMap<BasicBlock, ArrayInfo> blockArrayInfos;
    LinkedHashMap<StoreInst, Integer> storeCount;
    public LinkedHashMap<LoadInst, LinkedHashSet<StoreInst>> loadConflict;
    public LinkedHashMap<LoadInst, LinkedHashSet<Value>> loadCallopeConflict;
    ...
    public void runAnalysis(IRModule module, boolean needEliminate) {
        ...
        loadCallopeConflict = new LinkedHashMap<>();
        ...
        for (var bbnode : f.getBbs()) {
            ...
            for (var inst : info.mems) {
                if (inst instanceof LoadInst load) {
                    ...
                    loadCallopeConflict.put(load, new LinkedHashSet<>());
                    ...
                    for (var funcptr : info.memFuncsin.keySet()) {
                        ...
                        boolean isConflict = (AliasAnalysis.checkAlias(ghostPtradd, ptr) != AliasAnalysis.Alias.NO);
                        if(isConflict){
                            loadCallopeConflict.get(load).add(funcptr);
                        }
                        conflictWithFunc = conflictWithFunc || isConflict;
...
//                if (!ignoredArray.contains(AliasAnalysis.getRoot(store.getPointer()))) {
...
//                if (!ignoredArray.contains(AliasAnalysis.getRoot(load.getPointer()))) {
                info.mems.add(load);
//                }
            }
        }
