...
public class LCSSA implements Pass.IRPass {
    ...
    HashMap<BasicBlock, Phi> exitPhiMap = new HashMap<>();
    private void addPhiAtExitBbs(Instruction inst, IRLoop loop){
        ...
        for(BasicBlock exitBb : loop.getExitBlocks()) {
            HashSet<BasicBlock> domers = exitBb.getParentFunc().getDomer().get(exitBb);
        ...
        for(User user : userList){
            ...
            if(userBlock == instBb || loop.getBbs().contains(userBlock)){
                continue;
            }
    ...
    //  由于每个exitBlock都要对应一个phi value（草 这么简单的事情我居然想了一下午）
