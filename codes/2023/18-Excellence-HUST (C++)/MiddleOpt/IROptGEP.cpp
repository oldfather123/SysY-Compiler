...
#include "IROptGEP.h"
#include "IROptCF.h"
MiddleIR::Optimizer::IROptGEP::IROptGEP(const shared_ptr<MiddleIRAST>& irast)
    ...
    for (auto inst : bb->getInstructions()) {
        if (!inst->isGetElementPtrInst()) {
            ...
            if (i->getIndex().size() <= 1) {
                IR_ASSERT(
                    i->getIndex().size() == 1,
                    "getElementPtrInst's index size must be greater than 1"
                );
                newInsts.push_back(inst);
                continue;
            } else {
                ...
                shared_ptr<MiddleIRVal> lastAdd = nullptr;
                for (const auto& s : i->getIndex()) {
                    int64_t curShapeSizeBytes;
                    ...
                    // 先算出偏移量
                    auto mul_ = make_shared<IMathInst>(
                        IMathInst::IMathOp::MUL,
                        make_shared<IntType>(32),
                        s,
                        IR_INT_CONST(curShapeSizeBytes)
                    );
                    mul_->setName(G());
                    newInsts.push_back(mul_);
                    if (lastAdd != nullptr) {
                        // 再加到added上
                        auto add_ = make_shared<IMathInst>(
                            IMathInst::IMathOp::ADD, make_shared<IntType>(32), mul_, lastAdd
                        );
                        add_->setName(G());
                ...
                //     // 再加到added上
                //     auto add_ = make_shared<IMathInst>(
                //         IMathInst::IMathOp::ADD, make_shared<IntType>(32), mul_, added
                //     );
                //     add_->setName(G());
                //     newInsts.push_back(add_);
                //     added = add_;
