...
void sysY_asbuilder::module_gen() {
    ...
    asm_header += HAsm2Asm::space +".file" + HAsm2Asm::space + + "\"" + m_->get_module()->get_file_name() + "\"" + HAsm2Asm::newline;
    asm_header += HAsm2Asm::space + ".option" + HAsm2Asm::space + "pic" + HAsm2Asm::newline;
    m_->set_hasm_header(asm_header);

    for(auto func: m_->get_module()->get_functions()) {
...
void sysY_asbuilder::globals_def_gen() {
    for(auto var: m_->get_module()->get_global_variables()) {
        std::string global_var_def;
        std::string name = var->get_name();
        bool is_const = var->is_const();
        ...
        bool is_array = var->get_type()->get_pointer_element_type()->is_array_type();
        int size = var->get_type()->get_pointer_element_type()->get_size();
        global_variable_labels_table[var] = new Label(var->get_name());
        if(is_initialied) {
            if(is_array) {
                ...
                if(const_array->get_type()->get_array_element_type()->is_float_type()) {
                    for(int i = 0; i < const_array->get_size_of_array(); i++) {
                        global_var_def += HAsm2Asm::space + ".word" + HAsm2Asm::space + std::to_string(*(uint32_t*)&((static_cast<ConstantFP*>(const_array->get_element_value(i)))->get_value())) + HAsm2Asm::newline;
                    }
                } else {
                    for(int i = 0; i < const_array->get_size_of_array(); i++) {
                        global_var_def += HAsm2Asm::space + ".word" + HAsm2Asm::space + std::to_string((static_cast<ConstantInt*>(const_array->get_element_value(i)))->get_value()) + HAsm2Asm::newline;
                    }
                }
            } else {
                global_var_def += HAsm2Asm::space + ".globl" + HAsm2Asm::space + name + HAsm2Asm::newline;
                ...
                if(initializer->get_type()->is_float_type()) {
                    global_var_def += HAsm2Asm::space + std::to_string(*(uint32_t*)&(static_cast<ConstantFP*>(initializer)->get_value())) + HAsm2Asm::newline;
                } else {
                    global_var_def += HAsm2Asm::space + std::to_string(static_cast<ConstantInt*>(initializer)->get_value()) + HAsm2Asm::newline;
                }
            }
        } else {
            if(is_array) {
...
void sysY_asbuilder::function_gen() {
    //& for debug info
        ...
        for(auto &[val,interval]: fval2interval) {
            LOG(INFO) << "value: " << val->get_name() << ", " << "reg: " << Freg2name[interval->reg_id];
        }
    #endif
    ...
    func_header += HAsm2Asm::space + ".globl" + HAsm2Asm::space + func->get_name() + HAsm2Asm::newline;
    func_header += HAsm2Asm::space + ".type" + HAsm2Asm::space + func->get_name() + ",@function" + HAsm2Asm::newline;

    cur_func_->set_func_header(func_header);

    cur_bb_ = cur_func_->create_bb(func->get_entry_block(), bb2label[func->get_entry_block()]);
...
void sysY_asbuilder::linearizing_and_labeling_bbs() {
    bb2label.clear();
    linear_bbs.clear();
    auto func = cur_func_->get_function();
    std::list<BasicBlock*> linear_bbs_of_func = func->get_basic_blocks();
    ...
    for(auto bb: linear_bbs_of_func) {
        if(bb == func->get_entry_block() && bb->get_terminator()->is_ret()) {
            bb2label.insert({bb, new Label("")});
            linear_bbs.push_back(bb);
            return ;
        } else if(bb == func->get_entry_block()) {
            bb2label.insert({bb, new Label("")});
        } else if(bb != func->get_entry_block() && !bb->get_terminator()->is_ret()) {
            label_str = func->get_name() + "_" + bb->get_name();
...
int sysY_asbuilder::stack_space_allocation() {
    int total_size = 0;
    int iargs_size = 0;
    int fargs_size = 0;
    ...
    if(func->get_iargs().size() > 8) {
        int i = 0;
        for(auto arg: func->get_iargs()) {
            if(i >= 8) {
                int size_of_arg_type = arg->get_type()->get_size();
    ...
    if(func->get_fargs().size() > 8) {
        int i = 0;
        for(auto arg: func->get_fargs()) {
            if(i >= 8) {
                int size_of_arg_type = arg->get_type()->get_size();
    ...
    for(auto iter: ival2interval) {
        ...
        if(interval_iter->reg_id < 0) {
            ...
            if(arg && val2stack[static_cast<Value*>(arg)] != nullptr) 
                continue;
            int size_of_val_type = val_iter->get_type()->get_size();
    ...
    //& handle alloc inst
    for(auto &inst: func->get_entry_block()->get_instructions()) {
        auto alloc = dynamic_cast<AllocaInst*>(inst);
        if(!alloc)
            continue;
        int size_of_alloc_type = alloc->get_alloca_type()->get_size();
    ...
    //& for debug info
        ...
        for(auto &[val, loc]: val2stack) {
            std::cout << val->get_name() << ": " << loc->get_asm_code() << std::endl; 
        }
    #endif
...
void sysY_asbuilder::callee_stack_prologue(int stack_size) {
    int cur_offset = 0;
...
std::vector<std::pair<HAsmLoc*, HAsmLoc*>> sysY_asbuilder::callee_iargs_move(Function *func) {
    std::vector<std::pair<HAsmLoc*, HAsmLoc*>> to_move_locs;
    auto iargs_vector = func->get_iargs();
...
//! 初赛测试中未出现寄存器移动loop的情况
std::vector<std::pair<HAsmLoc*, HAsmLoc*>> sysY_asbuilder::callee_fargs_move(Function *func) {
    std::vector<std::pair<HAsmLoc*, HAsmLoc*>> to_move_locs;

    int iargs_num = func->get_iargs().size();
    iargs_num = iargs_num > 8 ? (iargs_num-8) : 0;

    auto fargs_vector = func->get_fargs();
...
void sysY_asbuilder::bb_gen() {
    Instruction *br_inst = nullptr;
    for(auto &inst: cur_bb_->get_bb()->get_instructions()) {
        ...
        if(inst->is_call()) {
            ...
            caller_reg_store(cur_bb_->get_bb()->get_parent(), call_inst);
            ...
            caller_reg_restore(cur_bb_->get_bb()->get_parent(), call_inst);
...
void sysY_asbuilder::callee_stack_epilogue(int stack_size) {
...
void sysY_asbuilder::ld_tmp_regs_for_inst(Instruction *inst) {
...
void sysY_asbuilder::alloc_tmp_regs_for_inst(Instruction *inst) {
    istore_list.clear();
    fstore_list.clear();
...
void sysY_asbuilder::store_tmp_reg_for_inst(Instruction *inst) {
    std::vector<std::pair<RegLoc*, RegBase*>> to_store_regs;
...
void sysY_asbuilder::tmp_regs_restore() {
...
void sysY_asbuilder::caller_reg_store(Function* func, CallInst* call) {
    ...
    int call_inst_reg = -1;
    bool is_float_call = call->get_type()->is_float_type();
    bool is_void_call = call->get_type()->is_void_type();
...
void sysY_asbuilder::caller_reg_restore(Function* func, CallInst* call) {
    std::vector<std::pair<RegLoc*, RegBase*>> to_restore_regs;
...
std::vector<std::pair<HAsmLoc*, HAsmLoc*>> sysY_asbuilder::caller_iargs_move(CallInst *call) {
    std::vector<std::pair<HAsmLoc*, HAsmLoc*>> to_move_locs;
    ...
    for(int i = iargs.size() - 1; i >= num_of_iargs; i--) {
        ...
        if(ival2interval.find(iargs[i]) != ival2interval.end()) {
            ...
            if(const_int) {
                to_move_locs.push_back(std::make_pair(new RegBase(reg_sp, extra_stack_offset + caller_trans_args_stack_offset), new ConstPool(const_int->get_value())));
            } else {
...
//! 初赛测试中未出现一个寄存器向多个寄存器中移动的情况
std::vector<std::pair<HAsmLoc*, HAsmLoc*>> sysY_asbuilder::caller_fargs_move(CallInst *call) {
    std::vector<std::pair<HAsmLoc*, HAsmLoc*>> to_move_locs;
    ...
    for(int i = fargs.size() - 1; i >= num_of_fargs; i--) {
        ...
        if(fval2interval.find(fargs[i]) != fval2interval.end()) {
            ...
            if(const_fp) {
                to_move_locs.push_back(std::make_pair(new RegBase(reg_sp, extra_stack_offset + caller_trans_args_stack_offset), new ConstPool(const_fp->get_value())));
            } else {
                LOG(ERROR) << "汇编代码生成发生错误";
            }
        }
...
void sysY_asbuilder::phi_union(Instruction *br_inst) {
    ...
    if(fcmpbr) {
        is_fcmpbr = true;   
        succ_bb = dynamic_cast<BasicBlock*>(fcmpbr->get_operand(2));
        fail_bb = dynamic_cast<BasicBlock*>(fcmpbr->get_operand(3));
    } else if(cmpbr) {
        is_cmpbr = true;
        succ_bb = dynamic_cast<BasicBlock*>(cmpbr->get_operand(2));
        fail_bb = dynamic_cast<BasicBlock*>(cmpbr->get_operand(3));
    } else {
        if(br_inst->get_num_operands() == 1) {
            succ_bb = dynamic_cast<BasicBlock*>(br_inst->get_operand(0));
        } else {
            succ_bb = dynamic_cast<BasicBlock*>(br_inst->get_operand(1));
            fail_bb = dynamic_cast<BasicBlock*>(br_inst->get_operand(2));
        }
    }

    for(auto sux: cur_bb_->get_bb()->get_succ_basic_blocks()) {
        bool is_succ_bb;
        if(sux == succ_bb) {
            is_succ_bb = true;
                ...
                for(auto opr: inst->get_operands()) {
                    if(dynamic_cast<BasicBlock*>(opr)) {
                        ...
                        if(dynamic_cast<ConstantFP*>(lst_val)) {
                            ...
                            auto src = new ConstPool(const_val->get_value());
    ...
    if(fcmpbr) {
        is_fcmpbr = true;   
        auto cond1 = fcmpbr->get_operand(0);
        auto cond2 = fcmpbr->get_operand(1);
        auto cmp_op = fcmpbr->get_cmp_op();
        succ_bb = dynamic_cast<BasicBlock*>(fcmpbr->get_operand(2));
        fail_bb = dynamic_cast<BasicBlock*>(fcmpbr->get_operand(3));
        ...
        switch(cmp_op) {
            case CmpOp::EQ: {
                    ...
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_fbeq(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                
                    } else if(const_cond1) {
                        if(fval2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_fbeq(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbeq(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(fval2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_fbeq(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbeq(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
            ...
            case CmpOp::GE: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_fbge(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                     
                    } else if(const_cond1) {
                        if(fval2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_fbge(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbge(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(fval2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_fbge(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbge(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(fval2interval[cond1]->reg_id < 0 && fval2interval[cond2]->reg_id < 0) {
            ...
            case CmpOp::GT: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_fbgt(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                     
                    } else if(const_cond1) {
                        if(fval2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_fbgt(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbgt(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(fval2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_fbgt(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbgt(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(fval2interval[cond1]->reg_id < 0 && fval2interval[cond2]->reg_id < 0) {
            ...
            case CmpOp::LE: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_fble(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                     
                    } else if(const_cond1) {
                        if(fval2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_fble(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fble(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(fval2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_fble(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fble(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(fval2interval[cond1]->reg_id < 0 && fval2interval[cond2]->reg_id < 0) {
            ...
            case CmpOp::LT: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_fblt(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                     
                    } else if(const_cond1) {
                        if(fval2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_fblt(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fblt(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(fval2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_fblt(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fblt(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(fval2interval[cond1]->reg_id < 0 && fval2interval[cond2]->reg_id < 0) {
            ...
            case CmpOp::NE: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_fbne(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                     
                    } else if(const_cond1) {
                        if(fval2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_fbne(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbne(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(fval2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_fbne(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_fbne(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(fval2interval[cond1]->reg_id < 0 && fval2interval[cond2]->reg_id < 0) {
        ...
        is_cmpbr = true;
        auto cond1 = cmpbr->get_operand(0);
        auto cond2 = cmpbr->get_operand(1);
        auto cmp_op = cmpbr->get_cmp_op();
        succ_bb = dynamic_cast<BasicBlock*>(cmpbr->get_operand(2));
        fail_bb = dynamic_cast<BasicBlock*>(cmpbr->get_operand(3));
        ...
        switch(cmp_op) {
            case CmpOp::EQ: {
                    //& 为了避免修改控制流可能造成的问题，不采取化简
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_beq(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                  
                    } else if(const_cond1) {
                        if(ival2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_beq(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_beq(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(ival2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_beq(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_beq(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
            ...
            case CmpOp::GE: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_bge(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                  
                    } else if(const_cond1) {
                        if(ival2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_bge(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_bge(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(ival2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_bge(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_bge(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(ival2interval[cond1]->reg_id < 0 && ival2interval[cond2]->reg_id < 0) {
            ...
            case CmpOp::LT: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_blt(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                  
                    } else if(const_cond1) {
                        if(ival2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_blt(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_blt(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(ival2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_blt(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_blt(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(ival2interval[cond1]->reg_id < 0 && ival2interval[cond2]->reg_id < 0) {
            ...
            case CmpOp::NE: {
                    if(const_cond1 && const_cond2) {
                        succ_br_inst = cur_bb_->create_bne(new Const(const_cond1->get_value()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                        cur_bb_->pop_inst();                  
                    } else if(const_cond1) {
                        if(ival2interval[cond2]->reg_id < 0) {
                            auto regbase = val2stack[cond2];
                            succ_br_inst = cur_bb_->create_bne(new Const(const_cond1->get_value()), new Mem(regbase->get_reg_id(), regbase->get_offset()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_bne(new Const(const_cond1->get_value()), get_asm_reg(cond2), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else if(const_cond2) {
                        if(ival2interval[cond1]->reg_id < 0) {
                            auto regbase = val2stack[cond1];
                            succ_br_inst = cur_bb_->create_bne(new Mem(regbase->get_reg_id(), regbase->get_offset()), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        } else {
                            succ_br_inst = cur_bb_->create_bne(get_asm_reg(cond1), new Const(const_cond2->get_value()), bb2label[succ_bb]);
                            cur_bb_->pop_inst();
                        }
                    } else {
                        if(ival2interval[cond1]->reg_id < 0 && ival2interval[cond2]->reg_id < 0) {
        ...
        if(br_inst->get_num_operands() == 1) {
            ...
            if(const_cond) {
                succ_br_inst = cur_bb_->create_bne(new Const(const_cond->get_value()), new Reg(reg_zero, false), bb2label[succ_bb]);
                cur_bb_->pop_inst();
...
std::vector<std::pair<HAsmLoc*, HAsmLoc*>> sysY_asbuilder::idata_move(std::vector<HAsmLoc*>&srcs, std::vector<HAsmLoc*>&dsts) {
...
std::vector<std::pair<HAsmLoc*, HAsmLoc*>> sysY_asbuilder::fdata_move(std::vector<HAsmLoc*>&srcs, std::vector<HAsmLoc*>&dsts) {
...
Reg *sysY_asbuilder::get_asm_reg(Value *val) {
...
void sysY_asbuilder::instr_gen(Instruction * inst) {
    ...
    switch(instr_type) {
        case Instruction::add: {
                ...
                if(const_op1 && const_op2) {
                    cur_bb_->create_add(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_add(get_asm_reg(inst), get_asm_reg(op2), new Const(const_op1->get_value()));
                } else if(const_op2) {
                    cur_bb_->create_add(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::sub: {
                ...
                auto const_op2 = dynamic_cast<ConstantInt*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_subw(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_subw(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_subw(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::mul: {
                ...
                auto const_op2 = dynamic_cast<ConstantInt*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_mulw(get_asm_reg(inst), new Const(const_op1->get_value()),  new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_mulw(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_mulw(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::mul64: {
                ...
                if(const_op1 && const_op2) {
                    ...
                    cur_bb_->create_mul64(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_mul64(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::sdiv: {
                ...
                auto const_op2 = dynamic_cast<ConstantInt*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_divw(get_asm_reg(inst), new Const(const_op1->get_value()),  new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_divw(get_asm_reg(inst), new Const(const_op1->get_value()),  get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_divw(get_asm_reg(inst), get_asm_reg(op1),  new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::srem: {
                ...
                auto const_op2 = dynamic_cast<ConstantInt*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_remw(get_asm_reg(inst), new Const(const_op1->get_value()),  new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_remw(get_asm_reg(inst), new Const(const_op1->get_value()),  get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_remw(get_asm_reg(inst), get_asm_reg(op1),  new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::asr: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                if(const_op1) {
                    cur_bb_->create_sraw(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else {
                    cur_bb_->create_sraw(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                }
            }
            break;
        ...
        case Instruction::shl: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                if(const_op1) {
                    cur_bb_->create_sllw(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else {
                    cur_bb_->create_sllw(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                }
            }
            break;
        case Instruction::lsr: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                if(const_op1) {
                    cur_bb_->create_srlw(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else {
                    cur_bb_->create_srlw(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                }
            }
            break;
        ...
        case Instruction::asr64: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                if(const_op1) {
                    cur_bb_->create_sra(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else {
                    cur_bb_->create_sra(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                }
            }
            break;
        ...
        case Instruction::shl64: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                if(const_op1) {
                    cur_bb_->create_sll(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else {
                    cur_bb_->create_sll(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                }
            }
            break;
        case Instruction::lsr64: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                if(const_op1) {
                    cur_bb_->create_srl(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else {
                    cur_bb_->create_srl(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                }
            }
            break;
        ...
        case Instruction::land: {
                ...
                if(const_op2 == nullptr)
                    LOG(ERROR) << "出现未预期情况";
                cur_bb_->create_land(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
            }
            break;
        ...
        case Instruction::fadd: {
                ...
                if(const_op1 && const_op2) {
                    cur_bb_->create_fadds(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_fadds(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_fadds(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::fsub: {
                ...
                auto const_op2 = dynamic_cast<ConstantFP*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_fsubs(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_fsubs(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_fsubs(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::fmul: {
                ...
                auto const_op2 = dynamic_cast<ConstantFP*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_fmuls(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_fmuls(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_fmuls(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::fdiv: {
                ...
                auto const_op2 = dynamic_cast<ConstantFP*>(op2);
                if(const_op1 && const_op2) {
                    cur_bb_->create_fdivs(get_asm_reg(inst), new Const(const_op1->get_value()), new Const(const_op2->get_value()));
                } else if(const_op1) {
                    cur_bb_->create_fdivs(get_asm_reg(inst), new Const(const_op1->get_value()), get_asm_reg(op2));
                } else if(const_op2) {
                    cur_bb_->create_fdivs(get_asm_reg(inst), get_asm_reg(op1), new Const(const_op2->get_value()));
                } else {
        ...
        case Instruction::fptosi:{
                ...
                if(const_src) {
                    cur_bb_->create_fcvtws(get_asm_reg(inst), new Const(const_src->get_value()));
                } else {
        ...
        case Instruction::sitofp: {
                ...
                if(const_src) {
                    cur_bb_->create_fcvtsw(get_asm_reg(inst), new Const(const_src->get_value()));
                } else {
        ...
        case Instruction::zext: 
            cur_bb_->create_zext(get_asm_reg(inst), get_asm_reg(inst->get_operand(0)));
            break;
        ...
        case Instruction::cmp: {
                ...
                auto cond1 = cmp_inst->get_operand(0);
                auto cond2 = cmp_inst->get_operand(1);
                auto const_cond1 = dynamic_cast<ConstantInt*>(cond1);
                auto const_cond2 = dynamic_cast<ConstantInt*>(cond2);
                
                if(const_cond2 && const_cond2->get_value() == 0) {
                    switch(cmp_op) {
                        case CmpOp::EQ:
                            if(const_cond1) {
                                cur_bb_->create_seqz(get_asm_reg(inst), new Const(const_cond1->get_value()));
                            } else {
                        ...
                        case CmpOp::NE:
                            if(const_cond1) {
                                cur_bb_->create_snez(get_asm_reg(inst), new Const(const_cond1->get_value()));
                            } else {
        ...
        case Instruction::fcmp: {
                ...
                auto cmp_op = (dynamic_cast<FCmpInst*>(inst))->get_cmp_op();
                auto const_cond1 = dynamic_cast<ConstantFP*>(cond1);
                auto const_cond2 = dynamic_cast<ConstantFP*>(cond2);
                switch (cmp_op) {
                    case CmpOp::EQ: {
                            if(const_cond1 && const_cond2) {
                                cur_bb_->create_feqs(get_asm_reg(inst), new Const(const_cond1->get_value()), new Const(const_cond2->get_value()));
                            } else if(const_cond1) {
                                cur_bb_->create_feqs(get_asm_reg(inst), new Const(const_cond1->get_value()), get_asm_reg(cond2));
                            } else if(const_cond2) {
                                cur_bb_->create_feqs(get_asm_reg(inst), get_asm_reg(cond1), new Const(const_cond2->get_value()));
                            } else {
                    ...
                    case CmpOp::GE: {
                            if(const_cond1 && const_cond2) {
                                cur_bb_->create_fges(get_asm_reg(inst), new Const(const_cond1->get_value()), new Const(const_cond2->get_value()));
                            } else if(const_cond1) {
                                cur_bb_->create_fges(get_asm_reg(inst), new Const(const_cond1->get_value()), get_asm_reg(cond2));
                            } else if(const_cond2) {
                                cur_bb_->create_fges(get_asm_reg(inst), get_asm_reg(cond1), new Const(const_cond2->get_value()));
                            } else {
                    ...
                    case CmpOp::GT: {
                            if(const_cond1 && const_cond2) {
                                cur_bb_->create_fgts(get_asm_reg(inst), new Const(const_cond1->get_value()), new Const(const_cond2->get_value()));
                            } else if(const_cond1) {
                                cur_bb_->create_fgts(get_asm_reg(inst), new Const(const_cond1->get_value()), get_asm_reg(cond2));
                            } else if(const_cond2) {
                                cur_bb_->create_fgts(get_asm_reg(inst), get_asm_reg(cond1), new Const(const_cond2->get_value()));
                            } else {
                    ...
                    case CmpOp::LE: {
                            if(const_cond1 && const_cond2) {
                                cur_bb_->create_fles(get_asm_reg(inst), new Const(const_cond1->get_value()), new Const(const_cond2->get_value()));
                            } else if(const_cond1) {
                                cur_bb_->create_fles(get_asm_reg(inst), new Const(const_cond1->get_value()), get_asm_reg(cond2));
                            } else if(const_cond2) {
                                cur_bb_->create_fles(get_asm_reg(inst), get_asm_reg(cond1), new Const(const_cond2->get_value()));
                            } else {
                    ...
                    case CmpOp::LT: {
                            if(const_cond1 && const_cond2) {
                                cur_bb_->create_flts(get_asm_reg(inst), new Const(const_cond1->get_value()), new Const(const_cond2->get_value()));
                            } else if(const_cond1) {
                                cur_bb_->create_flts(get_asm_reg(inst), new Const(const_cond1->get_value()), get_asm_reg(cond2));
                            } else if(const_cond2) {
                                cur_bb_->create_flts(get_asm_reg(inst), get_asm_reg(cond1), new Const(const_cond2->get_value()));
                            } else {
                    ...
                    case CmpOp::NE: {
                            if(const_cond1 && const_cond2) {
                                cur_bb_->create_fnes(get_asm_reg(inst), new Const(const_cond1->get_value()), new Const(const_cond2->get_value()));
                            } else if(const_cond1) {
                                cur_bb_->create_fnes(get_asm_reg(inst), new Const(const_cond1->get_value()), get_asm_reg(cond2));
                            } else if(const_cond2) {
                                cur_bb_->create_fnes(get_asm_reg(inst), get_asm_reg(cond1), new Const(const_cond2->get_value()));
                            } else {
        ...
        case Instruction::load: {
                ...
                if(global_addr) {
                    if(global_addr->get_type()->get_pointer_element_type()->is_float_type()) {
        ...
        case Instruction::loadoffset: {
                ...
                if(! dynamic_cast<GetElementPtrInst*>(loadoffset_inst->get_operand(0))) {
                    LOG(ERROR) << "汇编代码生成出现未预期情况";
                }
                auto offset = loadoffset_inst->get_offset();
                auto const_offset = dynamic_cast<ConstantInt*>(offset);
                if(loadoffset_inst->get_load_type()->is_float_type()) {
                    if(const_offset) {
                        cur_bb_->create_flw(get_asm_reg(inst), get_asm_reg(inst->get_operand(0)), new Const(const_offset->get_value()));
                    } else {
                        cur_bb_->create_flw(get_asm_reg(inst), get_asm_reg(inst->get_operand(0)), get_asm_reg(offset));
                    }
                } else {
                    if(const_offset) {
                        cur_bb_->create_lw(get_asm_reg(inst), get_asm_reg(inst->get_operand(0)), new Const(const_offset->get_value()));
                    } else {
                        cur_bb_->create_lw(get_asm_reg(inst), get_asm_reg(inst->get_operand(0)), get_asm_reg(offset));
                    }
                }
            }
            break;
        ...
        case Instruction::store: {
                ...
                auto const_int_src = dynamic_cast<ConstantInt*>(store_inst->get_operand(0));
                auto const_float_src = dynamic_cast<ConstantFP*>(store_inst->get_operand(0));
                if(global_addr) {
                    if(global_addr->get_type()->get_pointer_element_type()->is_float_type()) {
                        if(const_float_src) {
                            cur_bb_->create_fsw(new Const(const_float_src->get_value()), global_variable_labels_table[global_addr]);
                        } else {
                            cur_bb_->create_fsw(get_asm_reg(inst->get_operand(0)), global_variable_labels_table[global_addr]);
                        }
                    } else {
                        if(const_int_src) {
                            cur_bb_->create_sw(new Const(const_int_src->get_value()), global_variable_labels_table[global_addr]);
                        } else {
                            cur_bb_->create_sw(get_asm_reg(inst->get_operand(0)), global_variable_labels_table[global_addr]);
                        }
                    }                  
                } else {
                    LOG(ERROR) << "汇编代码生成出现异常";
        ...
        case Instruction::memset: {
                ...
                auto alloc_addr = dynamic_cast<AllocaInst*>(inst->get_operand(0));
                auto element_type = alloc_addr->get_alloca_type()->get_array_element_type();
                int type_size = element_type->get_size();
                auto addr = val2stack[alloc_addr];
                if(element_type->is_float_type()) {
                    cur_bb_->create_memset(addr, alloc_addr->get_alloca_type()->get_size(), type_size, true);
                } else if(element_type->is_integer_type()) {
                    cur_bb_->create_memset(addr, alloc_addr->get_alloca_type()->get_size(), type_size, false);
                } else {
                    LOG(ERROR) << "未预期情况发生";
                }
            }
        ...
        case Instruction::storeoffset: {
                ...
                auto offset = storeoffset_inst->get_offset();
                auto const_offset = dynamic_cast<ConstantInt*>(offset);
                auto const_int_src = dynamic_cast<ConstantInt*>(storeoffset_inst->get_operand(0));
                auto const_float_src = dynamic_cast<ConstantFP*>(storeoffset_inst->get_operand(0));
                if(storeoffset_inst->get_store_type()->is_float_type()) {
                    if(const_float_src) {
                        if(const_offset) {
                            cur_bb_->create_fsw(new Const(const_float_src->get_value()), get_asm_reg(inst->get_operand(1)), new Const(const_offset->get_value()));
                        } else {
                            cur_bb_->create_fsw(new Const(const_float_src->get_value()), get_asm_reg(inst->get_operand(1)), get_asm_reg(offset));
                        }
                    } else {
                        if(const_offset) {
                            cur_bb_->create_fsw(get_asm_reg(inst->get_operand(0)), get_asm_reg(inst->get_operand(1)), new Const(const_offset->get_value()));    
                        } else {
                            cur_bb_->create_fsw(get_asm_reg(inst->get_operand(0)), get_asm_reg(inst->get_operand(1)), get_asm_reg(offset));
                        }
                    }
                } else {
                    if(const_int_src) {
                        if(const_offset) {
                            cur_bb_->create_sw(new Const(const_int_src->get_value()), get_asm_reg(inst->get_operand(1)), new Const(const_offset->get_value()));
                        } else {
                            //! 在addi中使用了s1寄存器
                            cur_bb_->create_sw(new Const(const_int_src->get_value()), get_asm_reg(inst->get_operand(1)), get_asm_reg(offset));
                        }
                    } else {
                        if(const_offset) {
                            cur_bb_->create_sw(get_asm_reg(inst->get_operand(0)), get_asm_reg(inst->get_operand(1)), new Const(const_offset->get_value()));
                        } else {
                            cur_bb_->create_sw(get_asm_reg(inst->get_operand(0)), get_asm_reg(inst->get_operand(1)), get_asm_reg(offset));
                        }
                    }
                }
            }
        ...
        case Instruction::call: {
                cur_bb_->create_call(new Label(inst->get_operand(0)->get_name()));
            ...
            if(!inst->get_operands().empty()) {
                ...
                if(ret_val->get_type()->is_float_type()) {
                    if(const_float_ret_val) {
                        cur_bb_->create_callee_save_call_result(new RegLoc(reg_fa0, true), new Const(const_float_ret_val->get_value()));
                    } else {
                    ...
                    if(const_int_ret_val) {
                        cur_bb_->create_callee_save_call_result(new RegLoc(reg_a0, false), new Const(const_int_ret_val->get_value()));
                    } else {
