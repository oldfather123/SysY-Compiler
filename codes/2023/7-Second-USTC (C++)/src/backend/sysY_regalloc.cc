...
void RegAlloc::build_intervals(){
    for (auto bb_riter = block_order.rbegin(); bb_riter != block_order.rend();bb_riter++){
        ...
        int block_to = (*(instrs.rbegin()))->get_id()+2;
        //& for int, not alloc reg for pointer type
        for(auto iopr : bb->get_live_out_int()){
        ...
        //& for float
        for(auto fopr : bb->get_live_out_float()){
        ...
        for (auto instr_riter = instrs.rbegin(); instr_riter != instrs.rend();instr_riter++){
            ...
            if(!instr->is_void()){
                ...
                if(instr->get_type()->is_float_type()){
                    ...
                    fval2Inter[instr]->add_use_pos(instr->get_id());
                }
                //for int
                else{
                    ...
                    top_range->from = instr->get_id();
                    ival2Inter[instr]->add_use_pos(instr->get_id());
                }
            }
            ...
            for(auto opr : instr->get_operands()){
                ...
                if(opr->get_type()->is_float_type()){
                    if(fval2Inter.find(opr)==fval2Inter.end()){
                        ...
                        fval2Inter[opr]->add_range(block_from,instr->get_id()+2);
                        fval2Inter[opr]->add_use_pos(instr->get_id());
                    }
                    else{
                        auto fcur_interval = fval2Inter[opr];
                        fcur_interval->add_range(block_from,instr->get_id()+2);
                        fcur_interval->add_use_pos(instr->get_id());
                    }
                }
                //for int
                else{
                    if(ival2Inter.find(opr)==ival2Inter.end()){
                        ...
                        ival2Inter[opr]->add_range(block_from,instr->get_id()+2);
                        ival2Inter[opr]->add_use_pos(instr->get_id());
                    }
                    else{
                        auto icur_interval = ival2Inter[opr];
                        icur_interval->add_range(block_from,instr->get_id()+2);
                        icur_interval->add_use_pos(instr->get_id());
                }
                }
            }
        }
