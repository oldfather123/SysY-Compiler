...
public class AsmMin extends AsmInstruction {
    public AsmMin(IntRegister dest, IntRegister source1, IntRegister source2) {
        super(
    ...
    public String toString() {
        return String.format("AsmMin(%s, %s, %s)", getOperand(1), getOperand(2), getOperand(3));
    }

    @Override
    public String emit() {
        return String.format("\tmin %s, %s, %s\n", getOperand(1).emit(), getOperand(2).emit(), getOperand(3).emit());
    }
