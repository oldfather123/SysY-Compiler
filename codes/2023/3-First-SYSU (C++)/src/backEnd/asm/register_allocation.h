...
namespace RA
{
    ...
    void s_analyse_liveness(Func_Asm *func_asm)
    {
        ...
        cout<<func_asm->mbs.len<<endl;

        for (int i = 0; i < func_asm->mbs.len; i++)
        {

            cout<< "analyze use "<<i<<endl;
            for (auto I = func_asm->mbs[i]->inst; I; I = I->next)
            {
                Array<MOperand> defs = s_get_defs(I);
                Array<MOperand> uses = s_get_uses(I, func_asm->has_return_value);

                for (int i = 0; i < defs.len; i++)
                {
                    print_operand(defs[i]);
                    cout<<endl;
                }
                if (defs.len)
                    cout << "above is def---------" << defs.len << endl;

                for (int i = 0; i < uses.len; i++)
                {
                    print_operand(uses[i]);
                    cout<<endl;
                }
                if (uses.len)
                    cout << "above is use---------" << uses.len << endl;

                for (auto use : uses)
                {
                    if (use.tag == VSREG && def_set[i].find(use) == def_set[i].end())
                    {
                        use_set[i].insert(use);
                    }
                }
    ...
    Set<MI_VMove *, MI_VMove_Pointer_Cmp> s_coalesced_moves;
    Set<MI_VMove *, MI_VMove_Pointer_Cmp> s_constrained_moves;
    Set<MI_VMove *, MI_VMove_Pointer_Cmp> s_frozen_moves;
    Set<MI_VMove *, MI_VMove_Pointer_Cmp> s_worklist_moves;
    Set<MI_VMove *, MI_VMove_Pointer_Cmp> s_active_moves;
    ...
    Map<MOperand, Set<MI_VMove *, MI_VMove_Pointer_Cmp>> s_move_list;
    Map<MOperand, MOperand> alias;
    Map<MOperand, int32> color;
    Map<MOperand, int32> s_color;
...
#define in(it, set) (((set).find(it)) != ((set).end()))
    ...
    {
        return (opr.tag == SREG || opr.tag == VSREG);
    }

    void add_edge(MOperand u, MOperand v);
    void build(Func_Asm *func_asm);
    void s_build(Func_Asm *func_asm);
    ...
    void build(Func_Asm *func_asm)
    {
        ...
        for (auto b : func_asm->mbs)
        {
            int idx = b->i;
            // if(idx!=0)
            //     live_out[idx] =live_out[idx-1];
            auto live = live_out[b->i];
            // if(idx!=0)
            // {
            //     for(auto x:live_out[idx-1])
            //     {
    ...
    void s_build(Func_Asm *func_asm)
    {
        ...
        for (auto b : func_asm->mbs)
        {
            ...
            for (auto I = func_asm->mbs[b->i]->last_inst; I; I = I->prev)
            {
                ...
                auto uses = s_get_uses(I, func_asm->has_return_value);
                cout<< "I num is"<<mycnt++<<endl;
                if (I->tag == MI_VMOVE && defs.size()>0 && uses.size() > 0)
                {
                    cout<<I->tag<<endl;
                    if (s_need_alloc(defs[0]) && s_need_alloc(uses[0]))
                    {
                        ...
                        for (auto use : uses)
                        {
                            ...
                            print_operand(use);
                            cout<<endl;
                            // if(use.tag == SREG && use.value == s0)
                            //     continue;
                            live.erase(use);
                            move_list[use].insert((MI_Move *)I);
                        }

                        cout<<"above is use---------"<<endl;
    ...
    void s_coalesce()
    {
        ...
        else if (v.tag == SREG || in(e, adj_set))
        {
            constrained_moves.insert(m);
            s_add_worklist(u);
            s_add_worklist(v);
        }
        else if ((u.tag == SREG && okok) ||
                 (u.tag != SREG && conservative(join)))
        {
            coalesced_moves.insert(m);
            s_combine(u, v);
            s_add_worklist(u);
    ...
    void s_add_worklist(MOperand u)
    {
        if (u.tag != SREG && !move_related(u) && degree[u] < K)
        {
            freeze_worklist.erase(u);
            simplify_worklist.insert(u);
        }
    ...
    {
        auto p = Edge{t, r};
        return ((degree[t] < K) || (t.tag == SREG) || (in(p, adj_set)));
    }
    ...
    void select_spill()
    {
             ...
             it++)
        {
            ...
            real32 cost = use_def_count[*it] * pow(10, loop_depth[*it]) / degree[*it];
            print_operand(*it);
    ...
    void s_color_register(MOperand &m)
    {
        ...
        if (m.tag == SREG && s_is_callee_save(m.value))
        {
            if (needs_save.find(m.value) == -1)
            {
                needs_save.push(m.value);
    ...
    void allocate_register(Func_Asm *func_asm)
    {
                ...
                for (auto bb : func_asm->mbs)
                {
                    for (auto I = bb->inst; I; I = I->next)
                    {
                        ...
                        if (defs.find(n) != -1)
                        {
                            ...
                            /*
                            if (I->tag == MI_LOAD && ((MI_Load *)I)->mem_tag == MEM_LOAD_GLOBAL_REF) {
                                spilled_stack_size -= 4;
                                auto ldr_global = (MI_Load *) I;
                                I->mark(); // mark as dead
                            ...
                            */
                            ...
                            str->offset = make_imm(-(func_asm->stack_size + spilled_stack_size));
                        ...
                        if (uses.find(n) != -1)
                        {
                            ...
                            ldr->offset = make_imm(-(func_asm->stack_size + spilled_stack_size));
        ...
        else
        {
            ...
            if (!done_post_work)
            {
                ...
                cout << ">>> " << func_asm->name << "  " << func_asm->stack_size << endl;

                // insert add/sub to set up or destroy the stack
                if (func_asm->stack_size != 0)
                {
                    auto alloc_stack = new MI_Binary(BINARY_SUBTRACT, make_reg(sp), make_reg(sp), make_imm(func_asm->stack_size));
                    ...
                    for (auto bb : func_asm->mbs)
                    {
                        ...
                        {
                            auto destroy_stack = new MI_Binary(BINARY_ADD, make_reg(sp), make_reg(sp), make_imm(func_asm->stack_size));
                ...
                // not including the callee-save registers' space
                ...
                for (auto base : func_asm->local_array_bases)
                {
                    ...
                    int32 offset_relative_to_sp = func_asm->stack_size - sub->rhs.value;
                    ...
                    for (auto I = bb->inst; I; I = I->next)
                    {
                        ...
                        if (I->tag == MI_LOAD)
                        {
                            ...
                            if (ldr->mem_tag == MEM_LOAD_SPILL)
                            {
                                ...
                                int32 offset_relative_to_sp = func_asm->stack_size + offset_value;
                                ldr->base.value = sp;
                ...
                for (auto mb : func_asm->mbs)
                {
                    for (auto I = mb->inst; I; I = I->next)
                    {
                        ...
                        if (inst_need_legalize)
                        {
                            ...
                            // allocate a temp vreg to load the constant from literal pool
    ...
    void s_allocate_register(Func_Asm *func_asm)
    {
        //------------------------this is test---------------------
        Set<MOperand> s_initial;
        Set<MOperand> s_precolored;
        ...
        for (int i = 0; i < func_asm->vsreg_count; i++)
        {
            initial.insert(make_vsreg(i));
        }
        ...
        else
        {
            ...
            if (!done_post_work)
            {
                ...
                // // insert push/pops to save these registers
                needs_save.len=0;
                for(int reg = s1; reg <= s8 ;reg++)
                    needs_save.push(reg);
                if (!needs_save.empty())
                {
                    // insert push in the start block
                    std::sort(needs_save.begin(), needs_save.end());
                    auto store = new MI_VPush;
                    for (auto r : needs_save)
                    {
                        store->operands.push(make_sreg(r));
                    }
                    insert((MI *)store, func_asm->mbs[0]->inst);
                    ...
                    for (auto bb : func_asm->mbs)
                    {
                        if (bb->last_inst && bb->last_inst->tag == MI_RETURN)
                        {
                            auto restore = new MI_VPop;
                            for (auto r : needs_save)
                            {
                                restore->operands.push(make_sreg(r));
                            }
                            insert((MI *)restore, bb->last_inst);
                        }
                    }
                ...
                for (auto b : func_asm->mbs)
                {
                    for (auto I = func_asm->mbs[b->i]->inst; I; I = I->next)
                    {
                        ...
                        case MI_VMOVE:
                        {   
                            auto myobj2 = ((MI_VMove *)I)->dst;
                            if (myobj2.tag == VSREG || myobj2.tag == SREG)
                                s_color_register(((MI_VMove *)I)->dst);
                            auto myobj = ((MI_VMove *)I)->src;
                            if (myobj.tag == VSREG || myobj.tag == SREG)
                                s_color_register(((MI_VMove *)I)->src);
                        }
                        break;

                        case MI_VBINARY:
                        {
                            s_color_register(((MI_VBinary *)I)->dst);
                            s_color_register(((MI_VBinary *)I)->lhs);
                            if(((MI_VBinary *)I)->fneg != 1)
                                s_color_register(((MI_VBinary *)I)->rhs);
                        }
                        break;

                        case MI_VCOMPARE:
                        {
                            s_color_register(((MI_VCompare *)I)->lhs);
                            s_color_register(((MI_VCompare *)I)->rhs);
                        }
                        break;

                        case MI_VSTORE:
                        {
                            s_color_register(((MI_VStore *)I)->reg);
                            // oprs.push(((MI_VStore *)I)->base);
                            // oprs.push(((MI_VStore *)I)->offset);
                        }
                        break;

                        case MI_VLOAD: { s_color_register(((MI_VLoad *)I)->reg); } break;

                        case MI_VCVT:
                        {
                            s_color_register(((MI_VCvt *)I)->dst);
                            s_color_register(((MI_VCvt *)I)->src);
                        }
                        break;
                ...
                for (auto it = color.begin(); it != color.end(); it++)
                {
                    print_operand(it->first);
                    printf(" <- ");
                    print_operand(make_sreg(it->second));
                    printf("\n");
                }
                printf("\n");
            }
    ...
    void s_allocate_single_function(Func_Asm *func_asm, bool opt)
    {
        if (opt && func_asm->vsreg_count > 3000)
        {
...
void sregister_allocation(Program_Asm *program_asm, bool opt)
{
